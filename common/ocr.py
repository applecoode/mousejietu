...
def ocr_img_tess(image, config):
    ...
    # lang 指定中文简体
    region_texta = pytesseract.image_to_string(region_im, lang='chi_sim', config=tessdata_dir_config)
    region_text = region_texta.replace("_", "一").split("\n")
    region_text = region_texta.split(" ")
    ...
    return question, choices,region_texta

def ocr_img_baidu(image, config):
    # 百度OCR API  ，在 https://cloud.baidu.com/product/ocr 上注册新建应用即可
    """ 你的 APPID AK SK """
    APP_ID = '10694817'
    API_KEY = 'bCMLo2EWdfGfdolgoSq5DbOV'
    SECRET_KEY = 'mWZCQwoL8A86oZ4SgVclXPD8DGAw9O2O'
    ...
    # 处理出现问题为两行或三行
    if len(choices)==1:
        choices = re.split('[ABCDE]',choices[0])
        if '' in choices:
            choices.remove('')
    if choices[0].endswith('?'):
        question += choices[0]
